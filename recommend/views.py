...
class RecommendViewSet(ModelViewSet):
    ...
    def generate(self, request):
        ...
        if serializer.is_valid():
            ...
            # 국내, 해외, 국내|해외 여부에 따라 CSV 파일 선택
            if "국내" in input_nation and "해외" in input_nation:
            ...
            elif "국내" in input_nation:
                movies_data = pd.read_csv("static/korean.csv")
            elif "해외" in input_nation:
            ...
            for genre in input_genre.split("|"):
                selected_movies = pd.concat(
                    [
                        selected_movies,
            ...
            # 중복 제거, 가중치 컬럼 생성
            selected_movies = selected_movies.drop_duplicates(subset="번호")
            selected_movies["weight"] = 0

            # 유저가 로그인한 경우 선호 장르와 좋아요한 영화 정보 가져오기
            # accounts 모델 내용에 따라 달라질 예정
            """
            if request.user.is_authenticated:
                user = request.user
                user_genres = user.tag.split("|") if user.tag else []  # 선호장르 확인
                user_liked_movies = user.liked_movies.all().values_list(
                    "genre", flat=True  # 좋아요한 영화 확인
                )

                # 선호장르 가중치 부여
                for genre in user_genres:
                    selected_movies.loc[
                        selected_movies["kmdb장르"].str.contains(genre), "weight"
                    ] += 3

                # 영화 좋아요에 따른 가중치 부여
                for liked_genre in user_liked_movies:
                    selected_movies.loc[
                        selected_movies["kmdb장르"].str.contains(liked_genre), "weight"
                    ] += 1"""

            # 가중치, 관객수 기준으로 내림차순 정렬
            selected_movies = selected_movies.sort_values(
                by=["weight", "관객수"], ascending=[False, False]
            )

            # 상위 10개 중 랜덤으로 하나의 영화 선택
            top_10 = selected_movies.head(10)
            pick = top_10.sample(n=1)
            movie_title = pick.iloc[0]["kmdb제목"]
            movie_id = pick.iloc[0]["movie_id"]
            movie_seq = str(pick.iloc[0]["movie_seq"]).zfill(5)
            print(movie_title, movie_id, movie_seq)
